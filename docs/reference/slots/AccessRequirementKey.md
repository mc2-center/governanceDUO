...
- 'Deliberately an untyped string, not range: AccessRequirement: giving it a real
  ...
  The pattern above (matching AccessRequirement.id''s own slot_usage pattern in access_requirement.yaml)
  is the cycle-free substitute: it catches a malformed id without needing a typed
  range at all -- the same approach already used for entityIdList in that same file.
  See plans/identifier_update.md.'
